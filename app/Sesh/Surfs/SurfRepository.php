    ...
    public function findMatches(Collection $spots, \DateTime $date)
    {
        $matches = DB::select('
            SELECT 
              ...
              (
                1 - abs(todays_forecasts.swell_primary_absHeight - matching_forecasts.swell_primary_absHeight) 
                  / 
                  ((todays_forecasts.swell_primary_absHeight + matching_forecasts.swell_primary_absHeight) / 2)
              ) * 100 AS swell_height_match,
              (
                1 - abs(todays_forecasts.swell_primary_period - matching_forecasts.swell_primary_period) 
                / 
                ((todays_forecasts.swell_primary_period + matching_forecasts.swell_primary_period) / 2)
              ) * 100 AS swell_period_match,
              (
                1-abs(todays_forecasts.wind_speed - matching_forecasts.wind_speed) 
                / 
                ((todays_forecasts.wind_speed + matching_forecasts.wind_speed) / 2)
              ) * 100 as wind_speed_match
            ...
            JOIN msw_forecasts AS matching_forecasts ON (
              ...
              AND todays_forecasts.wind_speed <= matching_forecasts.wind_speed + 2
              /* Maths FTW https://stackoverflow.com/a/12234633 */
              AND (matching_forecasts.wind_trueDirection - todays_forecasts.wind_trueDirection + 180 + 360) 
                %
                360 - 180 >= -10
              AND 
                (matching_forecasts.wind_trueDirection - todays_forecasts.wind_trueDirection + 180 + 360) 
                %
                360 - 180 <= 10
              AND 
                (  
                  matching_forecasts.swell_primary_trueDirection 
                  - 
                  todays_forecasts.swell_primary_trueDirection + 180 + 360
                )
                % 
                360 - 180 >= -10
              AND 
        ...
        foreach ($matches as $match) {
            ...
            $return['matches'][$match->localTimestamp][$match->spot_id]['surfs'][$match->surf_id] = [
                ...
                'average_match' =>
                    ($match->swell_height_match + $match->wind_speed_match + $match->wind_speed_match) / 3,
            ];

            $return['refs']['forecasts'][$match->id] = $return['refs']['forecasts'][$match->id]
                ?? MswForecast::find($match->id);

            $return['refs']['spots'][$match->spot_id] = $return['refs']['spots'][$match->spot_id]
                ?? Spot::find($match->spot_id);

            $return['refs']['surfs'][$match->surf_id] = $return['refs']['surfs'][$match->surf_id]
                ?? Surf::with(['spot', 'mswForecast', 'photos'])->find($match->surf_id);
        }
